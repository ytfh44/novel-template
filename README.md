...
基于 Agent Skill 的小说创作模板项目。通过 8 个可调用的工作流，支持从构思到完稿的全流程管理。

## 快速开始

克隆下来此仓库，然后按以下顺序执行工作流：
...
- 默认运行环境为Antigravity
- 假如使用opencode等，需要把文件夹`.agent`改名为`.agents`
- 假如使用的是roo或kilo，需要把文件夹`.agent`改名为`.roo` 或 `.kilocode`，别的支持skills的Agent参照同理
- 但是这些skill实际上不依赖于这些Agent的内部特性，请放心使用。
- 在以模糊不清的自然语言作为拆分任务的机制的多Agent系统上（说的就是你，roo的orchestrator模式！），向来工作不佳。
